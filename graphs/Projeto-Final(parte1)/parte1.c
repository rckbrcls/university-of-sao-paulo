...
// seta os vertices que nao sao fontes
void buscarFonte(Graph* graph, int vert) {
    ...
    while (temp) { // enquanto a lista adjacente da posicao existir 
        ...
        graph->cor[adjvertice] = preto; // seta o vertice como percorrido
        
        temp = temp->prox; 
    }
}

// Conta o numero de vertices fontes
void contarFontes(Graph* graph){
    ...
    for(int i = 0; i < graph->numVert; i++){ // percorre todos os vertices
        if(graph->cor[i] == branco) // se um vertice nao foi setado como percorrido
            numFontes++; //  ele eh fonte
    }
